...
class StreamMergerTest extends WordSpecLike with MustMatchers {
  ...
  "Stream merger component" must {
    "merge single stream" in {
      val entries = Seq(
        JournalEntry(JournalId("1"), 0, PersistenceId("1"), 0, null),
        JournalEntry(JournalId("1"), 1, PersistenceId("1"), 1, null))
    ...
    "merge reordered stream" in {
      val entries = Seq(
        JournalEntry(JournalId("1"), 0, PersistenceId("1"), 0, null),
        JournalEntry(JournalId("1"), 1, PersistenceId("1"), 1, null),
        JournalEntry(JournalId("1"), 2, PersistenceId("1"), 3, null),
        JournalEntry(JournalId("1"), 3, PersistenceId("1"), 2, null))
    ...
    "merge multiple streams" in {
      ...
      val entries2 = Seq(
        JournalEntry(JournalId("2"), 0, PersistenceId("2"), 0, null),
        JournalEntry(JournalId("2"), 1, PersistenceId("2"), 1, null),
        JournalEntry(JournalId("2"), 2, PersistenceId("2"), 2, null))
      ...
      mergeResult must be((
        ...
        Map[PersistenceId, Long](PersistenceId("1") -> 1, PersistenceId("2") -> 2),
        correctEntries))
    }
    ...
    "merge multiple streams waiting on each other" in {
      ...
      val entries2 = Seq(
        ...
        JournalEntry(JournalId("2"), 2, PersistenceId("1"), 1, null))
      ...
      mergeResult must be((
        ...
        Map[PersistenceId, Long](PersistenceId("1") -> 2, PersistenceId("2") -> 2),
        correctEntries))
    }
